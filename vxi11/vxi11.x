 ...
 *
 * --CHANGES by Spencer E. Olson (July 23, 2008) in order to use rpcgen.py
 * from newpynfs to generate rpc protocol for python.
 * (3) Changed all longs (signed and unsigned) to (signed or unsigned) ints
 * (4) Changed all shorts ('') to ('') ints
 * (5) Changed chars (signed and unsigned) to (signed or unsigned) ints
 * (6) Gave specific values to enum Device_AddrFamily members, starting with 0
 *      at the first and 1 for the next.
 *
